...
export default class SkillBlock extends React.Component {
  ...
  openSkillsPurchaseScreen() {
    console.log('pressed');
    this.props.purchaseSkill(this.props.jedi);
  }
  ...
  render() {
    return (
      <TouchableOpacity onPress={() => this.openSkillsPurchaseScreen()}>
        <View style={styles.cardView}>
          <Card style={styles.card}
              title={this.props.jedi.skillName}
              image={{uri: this.props.jedi.photo}}
              imageStyle={{ flex: 1}}
              imageProps={{ resizeMode: 'contain'}}>
              <Text style={styles.textStyles}>
              Name: {this.props.jedi.seller}
              </Text>
              <Text style={styles.textStyles}>
