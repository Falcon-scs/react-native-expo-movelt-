...
*/
export default class MovingGroup extends React.Component {

  static navigationOptions = ({ navigation }) => {
  const params = navigation.state.params || {};
  const { navigate } = navigation;
  return {
    headerTitle: 'Find Moving Group',
    title: 'Find Moving Group',
    headerLeft: (
  ...
  async appendJedis(count, start) {
    ...
    firebase.database().ref('groups').orderByChild("Price").on('child_added', (snapshot) => {
    var childKey = snapshot.key;
    var childData = snapshot.val();
    childData.key = childKey;
    itemName = childData.sizeStorageUnit.toLowerCase();
  ...
  listItemRenderer(item) {
    return (
      <GroupBlock jedi={item}
      purchaseMovingGroup={this.purchaseMovingGroup}
  ...
  purchaseMovingGroup= async (item) => {
    this.props.navigation.navigate('MovingGroupPurchaseScreen', {item: item});
  }
